...
function OrionLib:MakeNotification(NotificationConfig)
	spawn(function()
		...
		local colors = self.Themes[self.SelectedTheme] or {
			Main = Color3.fromRGB(25,25,25),
			Second = Color3.fromRGB(50,50,50),
			Stroke = Color3.fromRGB(93,93,93),
			Text = Color3.fromRGB(255,255,255),
			TextDark = Color3.fromRGB(200,200,200),
			Timer = Color3.fromRGB(0,255,0)
		}
		...
		local NotificationFrame = SetChildren(SetProps(MakeElement("RoundFrame", colors.Main, 0, 10), {
			Parent = NotificationParent,
			Size = UDim2.new(1, 0, 1, 0),
			Position = UDim2.new(1, -55, 0, 0),
			BackgroundTransparency = 0
		}), {
			MakeElement("Stroke", colors.Stroke, 1.2),
			...
			SetChildren(SetProps(MakeElement("Label", NotificationConfig.Name, 15), {
			...
			SetChildren(SetProps(MakeElement("Label", NotificationConfig.Content, 14), {
			...
			SetChildren(SetProps(MakeElement("TFrame"), {
				...
				BackgroundColor3 = colors.Second,
				BorderSizePixel = 0,
		...
		local timerBar = NotificationFrame.TimerBackground.TimerBar
		TweenService:Create(NotificationFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Position = UDim2.new(0, 0, 0, 0)}):Play()
		TweenService:Create(timerBar, TweenInfo.new(NotificationConfig.Time, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 1, 0)}):Play()
...
function OrionLib:Init()
	if OrionLib.SaveCfg then
		local success, err = pcall(function()
			...
			if isfile(filePath) then
				...
				OrionLib:MakeNotification({
					Name = "設定読み込み成功",
					Content = "ゲーム " .. game.GameId .. " 用の設定を自動で読み込みました。",
					Time = 5
				})
			else
				writefile(filePath, "{}")
				OrionLib:MakeNotification({
					Name = "設定ファイル作成",
					Content = "ゲーム " .. game.GameId .. " 用の設定ファイルを新規作成しました。",
					Time = 5
				})
			end
		end)
		...
		if not success then
			OrionLib:MakeNotification({
				Name = "設定読み込み失敗",
				Content = "エラーが発生しました: " .. tostring(err),
				Time = 5
			})
		end
	end
...
-- Avatarフレーム作成
local AvatarFrame = SetChildren(SetProps(MakeElement("Frame"), {
...
-- Ownerアイコンを追加
if LocalPlayer.Name == "HfSapSatu" then
    SetChildren(AvatarFrame, {
        MakeElement("Image", "rbxassetid://95906008657764", { -- 王冠IDなどに置き換え
...
local WindowStuff = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
    ...
    SetChildren(SetProps(MakeElement("TFrame"), {
        ...
        AvatarFrame, -- ここで先に作ったAvatarFrameを入れる
        SetChildren(SetProps(MakeElement("TFrame"), {
            AnchorPoint = Vector2.new(0, 0.5),
            Size = UDim2.new(0, 32, 0, 32),
            Position = UDim2.new(0, 10, 0.5, 0)
        ...
        AddThemeObject(SetProps(MakeElement("Label", premiumText, 12), {
...
if WindowConfig.ShowIcon then
    ...
    local WindowIcon = SetProps(MakeElement("Image", WindowConfig.Icon), {
        Size = UDim2.new(0, 20, 0, 20),
        Position = UDim2.new(0, 25, 0, 2),  -- 少し上に寄せてTopBar内に収める
        AnchorPoint = Vector2.new(0, 0),    -- 左上基準
        ZIndex = 2,                         -- 他のUIより前面に
        Visible = true
    })
...
AddConnection(MinimizeBtn.MouseButton1Up, function()
	if Minimized then
		...
		TweenService:Create(MinimizeBtn.Ico, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Rotation = 360
		}):Play()
		...
		local shrinkTween = TweenService:Create(MainWindow, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
...
function Slider:Set(Value)
    ...
    if SliderBar.Value then SliderBar.Value.Text = tostring(self.Value).." "..SliderConfig.ValueName end
    if SliderDrag.Value then SliderDrag.Value.Text = tostring(self.Value).." "..SliderConfig.ValueName end
    SliderConfig.Callback(self.Value)
end

-- 2. 加算
function Slider:Increment(Value)
    self:Set(self.Value + Value)
end

-- 3. 減算
function Slider:Decrement(Value)
    self:Set(self.Value - Value)
end

-- 4. 最大値
function Slider:SetMax()
    self:Set(SliderConfig.Max)
end

-- 5. 最小値
function Slider:SetMin()
    self:Set(SliderConfig.Min)
end

-- 6. 中間値
function Slider:SetMiddle()
    self:Set((SliderConfig.Max + SliderConfig.Min)/2)
end

-- 7. パーセンテージで設定 (0～1)
function Slider:SetPercent(Percent)
    self:Set(SliderConfig.Min + (SliderConfig.Max - SliderConfig.Min) * math.clamp(Percent, 0, 1))
end

-- 8. ランダム値
function Slider:SetRandom()
    local step = SliderConfig.Increment
    local range = math.floor((SliderConfig.Max - SliderConfig.Min)/step)
    self:Set(SliderConfig.Min + step * math.random(0, range))
end

-- 9. 初期値にリセット
function Slider:Reset()
    self:Set(SliderConfig.Default)
end

-- 10. 値を倍にする (最大値を超えた場合は最大値にクリップ)
function Slider:Double()
    self:Set(self.Value * 2)
end

-- 11. 半分にする (最小値未満は最小値にクリップ)
function Slider:Half()
    self:Set(self.Value / 2)
end

-- 12. 増分ステップで次に進める
function Slider:NextStep()
    self:Increment(SliderConfig.Increment)
end

-- 13. 減分ステップで戻す
function Slider:PrevStep()
    self:Decrement(SliderConfig.Increment)
end

-- 14. 最小～最大までの割合を返す (0～1)
function Slider:GetPercent()
    return (self.Value - SliderConfig.Min) / (SliderConfig.Max - SliderConfig.Min)
end

-- 15. 値を丸めて返す
function Slider:GetRounded()
    return Round(self.Value, SliderConfig.Increment)
end
